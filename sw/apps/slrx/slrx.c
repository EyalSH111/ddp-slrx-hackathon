...
#ifdef FUSED_XON
        // Conv0 + Pool0: fused in HW — writes directly to post_p0_fm, skips post_c0_fm
        conv_xlr_setup_fused(
             (uint8_t*)(slr_intr_fm_p->post_p0_fm),         // output: pool0 feature-map (direct)
             (uint8_t*) slr_img,                            // input: raw image
                        IMG_DIM,                            // input dimension
             (int8_t*) (slr_model_params_p->conv0_w),       // kernel weights
             (int32_t) (slr_model_params_p->conv0_b));      // kernel bias
        if (check_performance) report_task_performance("Conv0");
        if (check_performance) report_task_performance("Pool0"); // 0 cycles: fused above
#else
        // Calling CONV0 Layer
...
#ifdef FUSED_XON
        // Conv1 + Pool1: fused in HW — writes directly to post_p1_fm, skips post_c1_fm
        conv_xlr_setup_fused(
             (uint8_t*)(slr_intr_fm_p->post_p1_fm),         // output: pool1 feature-map (direct)
             (uint8_t*)(slr_intr_fm_p->post_p0_fm),         // input: pool0 output
                        POST_P0_FM_DIM,                     // input dimension
             (int8_t*) (slr_model_params_p->conv1_w),       // kernel weights
             (int32_t) (slr_model_params_p->conv1_b));      // kernel bias
        if (check_performance) report_task_performance("Conv1");
        if (check_performance) report_task_performance("Pool1"); // 0 cycles: fused above
#else
        // Calling CONV1 Layer
